...
class GeniusMailer
{
    ...
    public function sendAutoOrderMail(array $mailData,$id)
    {
        ...
        try{
            Mail::send('admin.email.order_emailbody',$data, function ($message) use ($objDemo,$id) {
                $message->from($objDemo->from,$objDemo->title);
                $message->to($objDemo->to);
                $message->subject($objDemo->subject);
        // $order = Order::findOrFail($id);
        // $cart = unserialize(bzdecompress(utf8_decode($order->cart)));
        // $fileName = public_path('assets/print/').str_random(4).time().'.pdf';
        ...
        $files = glob('assets/print/*'); //get all file names
