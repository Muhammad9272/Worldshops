...
class CheckoutController extends Controller
{
    ...
    public function checkoutnote(Request $request)
    {
        if($request->checkout_note){
            if(Session::has('checkout_note')) {
              Session::forget('checkout_note');
            } 
          Session::put('checkout_note',$request->checkout_note);
        }
    ...
    public function checkout()
    {
        $curr = Currency::where('is_default','=',1)->first();       
        if (!Session::has('cart')) {
            return redirect()->route('front.cart')->with('chkoutmsg',"You don't have any product to checkout.");
        }
            ...
            foreach(Session::get('cart')->items as $product){
                $user_id=$product['item']['user_id'];
                break;
            }
            foreach(Session::get('cart')->items as $product){
                $subtotal += $product['price'];
            }   
            $produser=User::where('id',$user_id)->first();
            // dd($produser->min_delivery);
            if($subtotal<$produser->min_delivery){
                
                $msg="Minimum Order Value must be ".$curr->sign.''.$produser->min_delivery;
                $msg = str_replace("\r\n", "", $msg);
                return redirect()->route('front.cart')->with('chkoutmsg', $msg ); 
            }
        ...
        {    
            $chkoutpg=1;
            $shipaddress=AddressBook::where('user_id',Auth::user()->id)->orderBy('id', 'desc')->get();
            $shipadd=AddressBook::where('user_id',Auth::user()->id)->orderBy('id', 'desc')->first();
            // $billingaddress=AddressBook::where('user_id',Auth::user()->id)->where('address_type',1)->get();
             $usercard = UserCreditCard::where([ ['primary',1],['user_id',Auth::id()] ])->first();
              $usercards = UserCreditCard::where('user_id',Auth::id())->orderBy('primary', 'desc')->get();
            $countries=Country::all();
            return view('front.checkout',compact('curr','shipaddress','countries','chkoutpg','shipadd','usercard','usercards'));
             ...
             $checked=1;
             $countries=Country::all();
            return view('front.checkout',compact('curr','checked','countries','chkoutpg'));
        }
    ...
    public function cashondelivery(Request $request)
    {
        ...
        $order['customer_address_1'] = $request->address2;
