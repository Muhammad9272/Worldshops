...
class CartController extends Controller
{
    ...
    public function cartview($ck=null)
    {   
        if($ck==1){
            $chkoutpg=$ck;
        }
        ...
        return view('includes.front.bucket-load',compact('chkoutpg')); 
    }
   ...
   public function addcart($id)
    ...
    public function addnumcart()
    {
        ...
        $methodofcollect= $_GET['methodofcollect'];
        $sshipping_cost=0;

        $size_price = ($size_price / $curr->value);
        $prod = Product::where('id','=',$id)->first(['id','user_id','slug','name','photo','size','size_qty','size_price','color','price','stock','type','file','link','license','license_qty','measure','whole_sell_qty','whole_sell_discount','attributes','buy_quantity','discount_check','offer_amount']);
       ...
       if($prod->user_id != 0){
        ...
        $prc = $prod->price + $gs->fixed_commission + ($prod->price/100) * $gs->percentage_commission ;
        $prod->price = round($prc,2);
            if($methodofcollect==1){
                $sellerd=User::where('id',$prod->user_id)->first();
                $sshipping_cost=$sellerd->shipping_cost;   
            }
            else{
                 $sshipping_cost=0;
            }
        ...
        $cart->methodofcollect=$methodofcollect;
        $cart->sshipping_cost=$sshipping_cost;
        $cart->gov_bag_charges=$gs->gov_bag_charges;
        $cart->service_charges=$gs->service_charges;

        $cart->totalPrice = 0;
        foreach($cart->items as $data)
        $cart->totalPrice += $data['price'];   

        $cart->totalPrice+=$gs->gov_bag_charges+$gs->service_charges+$sshipping_cost;
        $mainTotal1=$cart->totalPrice;
        $cart->mainTotal1=$mainTotal1;

        Session::put('cart',$cart);
        $data[4]=$cart->totalQty;
        // $data[0] = count($cart->items);   
        return response()->json($data);        
    }  
    ...
    public function addbyone()
    {
        ...
        $prod = Product::where('id','=',$id)->first(['id','user_id','category_id','slug','name','photo','size','size_qty','size_price','color','price','stock','type','file','link','license','license_qty','measure','whole_sell_qty','whole_sell_discount','attributes','buy_quantity','discount_check','offer_amount']);
    ...
    public function shipchanges($id)
    {
           $gs = Generalsetting::findOrFail(1);
           if (Session::has('coupon')) {
                Session::forget('coupon');
         ...
         if(Session::has('shop_name')){
            $user=User::where('shop_name',Session::get('shop_name') )->first(); 
            $sshipping_cost=$user->shipping_cost;      
         }    
         else{
            $data = 0;
            return response()->json($data);
        ...
        if (isset($cart->items) && count($cart->items) > 0) {
            ...
            $sshipping_cost=0;
                ...
                if($methodofcollect==1){
                    $sellerd=User::where('shop_name',Session::get('shop_name') )->first();
                    $sshipping_cost=$sellerd->shipping_cost;   
                }
                else{
                     $sshipping_cost=0;
    ...
    public function coupon()
    {
        ...
        // $total = (float)preg_replace('/[^0-9\.]/ui','',$_GET['total']);;
         if(Session::has('shop_name')){
            $selleruser=User::where('shop_name',Session::get('shop_name') )->first(); 
                
         }
         else{
              return response()->json(0);
         }
        $fnd = Coupon::where('code','=',$code)->whereIn('user_id',['0',$selleruser->id])->get()->count();
        if($fnd < 1)
        {
        return response()->json(0);              
        }
        else{
        $coupon = Coupon::where('code','=',$code)->whereIn('user_id',['0',$selleruser->id])->first();
        ...
        $from = date('Y-m-d H:i:s',strtotime($coupon->start_date));
        $to = date('Y-m-d H:i:s',strtotime($coupon->end_date));
        if($from <= $today && $to >= $today)
        {
            if($coupon->status == 1)
            {
                ...
                $charges=$gs->gov_bag_charges+$gs->service_charges+$cart->sshipping_cost;
                $total= $cart->totalPrice;

                if($total<$coupon->min_value){
                    return response()->json(33);
                }
                ...
                if($coupon->type == 0)
                {
                    ...
                    $total = $total+$charges - $sub;
                    $data[0] = round($total,2);
                    if($gs->currency_format == 0){
                        $data[6]=$curr->sign.(Product::servicecharges() + $data[0] );
                        $data[0] = $curr->sign.$data[0];
                        
                    }
                    else{
                        $data[6]=(Product::servicecharges() + $data[0] ).$curr->sign;
                        $data[0] = $data[0].$curr->sign;
                    ...
                    Session::put('min_value', $coupon->min_value);
                    ...
                    Session::put('already', $code);
                    $total = $total+$charges - round($coupon->price * $curr->value, 2);
                    $data[0] = round($total,2);
                    if($gs->currency_format == 0){
                        $data[6]=$curr->sign.(Product::servicecharges() + $data[0] );
                        $data[0] = $curr->sign.$data[0];
                    ...
                    return response()->json($data);              
                     ...
                     if($cart->methodofcollect==0){
                        return response()->json(35);
                     }
                    Session::put('already', $code);
                    $charges-=$cart->sshipping_cost;
