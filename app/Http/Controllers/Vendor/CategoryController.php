...
class CategoryController extends Controller
{
    ...
    {    $user_id=Auth::user()->id;
         $datas = Category::where('user_id',$user_id)->orderBy('position','asc')->get();
         ...
         return Datatables::of($datas)
                            ->addColumn('status', function(Category $data) {
                                ...
                                return '<div class="action-list"><select class="process select droplinks '.$class.'"><option data-val="1" value="'. route('vendor-cat-status',['id1' => $data->id, 'id2' => 1]).'" '.$s.'>Activated</option><option data-val="0" value="'. route('vendor-cat-status',['id1' => $data->id, 'id2' => 0]).'" '.$ns.'>Deactivated</option>/select></div>';
                            })
                            
                            ->addColumn('action', function(Category $data) {
                                return '<div class="action-list">
                                <a data-href="' . route('vendor-cat-edit',$data->id) . '" class="edit" data-toggle="modal" data-target="#modal1"> <i class="fas fa-edit"></i>Edit</a>

                                  <a href="javascript:;" data-href="' . route('vendor-cat-delete',$data->id) . '" data-toggle="modal" data-target="#confirm-delete" class="delete"><i class="fas fa-trash-alt"></i></a></div>';
                            })
    ...
    {   
        ...
        $user_id=Auth::user()->id;
        $data = Category::where('user_id',$user_id)->findOrFail($id);
    ...
    public function update(Request $request, $id)
    {
      ...
      {
          $user_id=Auth::user()->id;
          $data = Category::where('user_id',$user_id)->findOrFail($id1);
          $data->status = $id2;
          $data->update();
      }
    ...
    {  
        ...
        foreach ($cats as $item) {
            return Category::where('id', '=', $id)->update(array('position' => $sorting));
        }
