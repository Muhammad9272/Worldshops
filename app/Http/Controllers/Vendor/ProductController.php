...
class ProductController extends Controller
{
    ...
    {
         $user = Auth::user();
         $datas = $user->products()->where('product_type','normal')->orderBy('name','asc')->get();
         ...
         return Datatables::of($datas)
                            ...
                            ->addColumn('action', function(Product $data) {
                                ...
                                <a data-price="'.$data->price.'" data-href="' . route('vendor-prod-updatePrice',$data->id) . '" class="editprice" data-toggle="modal" data-target="#modalprice1"> <i class="fas fa-edit"></i>Price</a>

                                <a href="' . route('vendor-prod-edit',$data->id) . '"> <i class="fas fa-edit"></i>'.$this->vendor_language->lang715.'</a>
                                <a href="javascript:;" data-href="' . route('vendor-prod-delete',$data->id) . '" data-toggle="modal" data-target="#confirm-delete" class="delete"><i class="fas fa-trash-alt"></i></a></div>';
                            })
    ...
    //*** JSON Request
    // public function datatablesdefective()
    // {
    //      $user = Auth::user();
    //      $datas = $user->products()->where('product_type','normal')->orderBy('position','asc')->get();
   ...
   //*** GET Request
    ...
    public function prodcatwise()
    {
         $user = Auth::user();
         $datas = Category::whereIn('user_id',[0,$user->id])->orderBy('position', 'ASC')->where('status',1)->get();
         return view('vendor.product.productcat.cat',compact('datas'));
    }

    public function prodsubcatwise($cat)
    {
     $data=Category::where('slug',$cat)->first();

        if($data->subs()->count()>0){
         return view('vendor.product.productcat.subcat',compact('data'));
        }
        else{

            return redirect()->route('vendor-prod-index1',$data->slug);
        }
    
    }

    public function prodchildcatwise($cat,$sub)
    {
     $data=Subcategory::where('slug',$sub)->first();

        if($data->childs()->count()>0){
           return view('vendor.product.productcat.childcat',compact('data'));
        }  
        else{
             return redirect()->route('vendor-prod-index1',[$cat,$data->slug]);
        }
    ...
    public function index1($slug=null, $slug1=null, $slug2=null)
    { 
      $user = Auth::user();
      $cat = null;
      $subcat = null;
      ...
      if (!empty($slug)) {
        $cat = Category::where('slug', $slug)->whereIn('user_id', [0,$user->id])->firstOrFail();
        $data['cat'] = $cat;
      }
      if (!empty($slug1)) {
        $subcat = Subcategory::where('slug', $slug1)->whereIn('user_id', [0,$user->id])->firstOrFail();
        $data['subcat'] = $subcat;
      }
      if (!empty($slug2)) {
        $childcat = Childcategory::where('slug', $slug2)->whereIn('user_id', [0,$user->id])->firstOrFail();
        $data['childcat'] = $childcat;
                                ...
                                $prods = Product::when($cat, function ($query, $cat) {
                                  ...
                                  $datas = $prods->where('user_id',$user->id)->where('product_type','normal')->get();

        return view('vendor.product.catalogs',compact('datas'));
    }
    public function index()
    {
     ...
     public function indexdefective()
    {   
        $ids=[];
        $user = Auth::user();
        ...
        {
            $query->where('subcategory_id',null)
            ->orWhere('childcategory_id',null);
        })
        ->orderBy('position', 'asc')->get();

        // foreach ($datas as $key => $data) {
        //    if(   ($data->category->subs->count()>0 && is_null($data->subcategory_id) ) ||
        //          ($data->subcategory->childs->count()>0 && is_null($data->childcategory_id) )
        //       ) {
        ...
        return view('vendor.product.defective_products',compact('datas'));
    }
    ...
    {   $user_id=Auth::user()->id;
        $cats = Category::whereIn('user_id',[0,$user_id])->where('status',1)->get();
        $sign = Currency::where('is_default','=',1)->first();
    ...
    public function importSubmit(Request $request)
    {
        ...
        while (($line = fgetcsv($file)) !== FALSE) {
            ...
            if (!Product::where('sku',$line[0])->exists()){
                ...
                $mcat = Category::where(DB::raw('lower(name)'), strtolower($line[1]))->whereIn('user_id', [0,$user->id]);
                ...
                if($mcat->exists()){
                    ...
                    if($line[2] != ""){
                        $scat = Subcategory::where(DB::raw('lower(name)'), strtolower($line[2]))->whereIn('user_id', [0,$user->id]);
                    ...
                    if($line[3] != ""){
                        $chcat = Childcategory::where(DB::raw('lower(name)'), strtolower($line[3]))->whereIn('user_id', [0,$user->id]);
                ...
                if($line[5] != ""){
                $input['photo'] = $line[5];}
                $input['name'] = utf8_encode($line[4]);
                $input['details'] = utf8_encode($line[6]);
                ...
                // $img1 = Image::make($line[5]);
                // $thumbnail1 = time().str_random(10).'.jpg';
                // $img1->save(public_path().'/assets/images/products/'.$thumbnail1);
                // $prod->photo  = $thumbnail1;
                // $prod->update();
        ...
        if (!empty($request->meta_tag))
         {
           ...
           if (!$request->discount_check) {
             $input['discount_check'] = 0;
           } 
...
// PRODUCT DELETE ENDS
    ...
    public function exportCsv(Request $request)
    {
     $fileName = 'products.csv';
      ...
      $user = Auth::user();
      $products = $user->products()->where('product_type','normal')->orderBy('category_id','asc')->get();
        ...
        $columns = array('sku', 'main_category', 'sub_category', 'child_category', 'product_name','product_feature_image','product_description','current_price','previous_price','stock','policy','product_type');
        ...
        $callback = function() use($products, $columns) {
            ...
            foreach ($products as $prod) {
                $row['sku']  = $prod->sku;
                $row['main_category']    = $prod->category?$prod->category->name:'';
                $row['sub_category']    = $prod->subcategory?$prod->subcategory->name:'';
                $row['child_category']  = $prod->childcategory?$prod->subcategory->name:'';
                $row['product_name']  = $prod->name;
                $row['product_feature_image']    = url('assets/images/products/'.$prod->photo);
                $row['product_description']    = strip_tags($prod->details);
                $row['current_price']  = $prod->price;

                $row['previous_price']  = $prod->previous_price>0?$prod->previous_price:'';
                $row['stock']  = $prod->stock;
                $row['policy']  = strip_tags($prod->policy);
                $row['product_type']  ="normal";


                fputcsv($file, array($row['sku'], $row['main_category'], $row['sub_category'], $row['child_category'], $row['product_name'], $row['product_feature_image'], $row['product_description'], $row['current_price'], $row['previous_price'], $row['stock'], $row['policy'], $row['product_type']  ));
            }
    ...
    {
       $user_id=Auth::user()->id;
        $data = Product::where('user_id',$user_id)->findOrFail($id);
        $sign = Currency::where('is_default','=',1)->first();
        return view('vendor.product.editprice',compact('data','sign'));
    }
    
    public function updatePrice(Request $request, $id)
    { 
        $user_id=Auth::user()->id;
        $data = Product::where('user_id',$user_id)->where('id',$id)->first();
