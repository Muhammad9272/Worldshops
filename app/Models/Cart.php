...
class Cart extends Model
{
    ...
    public $methodofcollect = 0;

    public $sshipping_cost = 0;
    public $service_charges = 0;
    public $gov_bag_charges = 0;
    ...
    public function __construct($oldCart)
    {
        if ($oldCart) {
            ...
            $this->totalPrice = $oldCart->totalPrice;
             $this->mainTotal1=$oldCart->mainTotal1;

            $this->sshipping_cost = $oldCart->sshipping_cost;
            $this->service_charges = $oldCart->service_charges;
            $this->gov_bag_charges = $oldCart->gov_bag_charges;

            $this->methodofcollect = $oldCart->methodofcollect;
...
// **************** ADD TO CART *******************
    ...
    public function add($item, $id, $size,$color, $keys, $values) {
        $size_cost = 0;
        $storedItem = ['qty' => 0,'free_prod'=>0,'size_key' => 0, 'size_qty' =>  $item->size_qty,'size_price' => $item->size_price, 'size' => $item->size, 'color' => $item->color, 'stock' => $item->stock, 'price' => $item->price, 'item' => $item, 'license' => '', 'dp' => '0','keys' => $keys, 'values' => $values];
...
// **************** ADDING QUANTITY *******************
    ...
    public function adding($item, $id, $size_qty, $size_price) {
        $storedItem = ['qty' => 0,'free_prod'=>0,'size_key' => 0, 'size_qty' =>  $item->size_qty,'size_price' => $item->size_price, 'size' => $item->size, 'color' => $item->color, 'stock' => $item->stock, 'price' => $item->price, 'item' => $item, 'license' => '', 'dp' => '0','keys' => '', 'values' => ''];
        if ($this->items) {
      ...
      if($item->discount_check==1 && $storedItem['qty']>=$item->buy_quantity){

        $remain=fmod($storedItem['qty'], $item->buy_quantity);
        $free_quant=(int)($storedItem['qty']/$item->buy_quantity);
        $storedItem['price']=($free_quant*$item->offer_amount)+($remain*$item->price);
      }
      else{
        $storedItem['price'] = $item->price * $storedItem['qty'];
      }
...
// **************** REDUCING QUANTITY ENDS *******************
    ...
    public function shipchange($id,$sshipping_cost,$coupon_price) {
        $gs = Generalsetting::findOrFail(1);
         
        if($id==1){

          $this->totalPrice += $sshipping_cost;
          $this->totalPrice += $coupon_price;
          $this->sshipping_cost = $sshipping_cost;
          $this->methodofcollect=1;
        }
        if($id==0){
          ...
          $this->sshipping_cost = 0;
          $this->methodofcollect=0;
        }    
