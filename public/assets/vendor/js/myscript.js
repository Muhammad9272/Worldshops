...
// **************************************  AJAX REQUESTS SECTION *****************************************
  ...
  // Status Ends
      ...
      $(document).on('click','.sendToCourierbtn',function () {
        ...
        var link = $(this).attr('data-href');
            ...
            $.get( link, function(data) {
                 ...
                 if(data['0']==1){
                  $('.alert-danger').hide();
                  $('.alert-success').show();
                  $('.alert-success p').html(data[1]);
                  console.log(12);
                 }
                 else{
                  $('.alert-success').hide();
                  $('.alert-danger').show();
                  $('.alert-danger p').html(data[1]);
                  console.log(23);
                 }
...
// ADD / EDIT FORM SUBMIT FOR DATA TABLE
    ...
    $('#modalprice1').on('show.bs.modal', function(e) {
        $(this).find('.prodprice').val($(e.relatedTarget).attr('data-price'));
        $(this).find('#updateprice').attr('action', $(e.relatedTarget).data('href'));
    });


    $(document).on('submit','#updateprice',function(e){
        e.preventDefault();
          ...
          $.ajax({
           ...
           success:function(data)
           {
              ...
              $('.alert-danger ul').html('');
                ...
                $("#modalprice1 .modal-content .modal-body .alert-danger").focus();
                $('button.addProductSubmit-btn').prop('disabled',false);
                $('#updateprice input , #updateprice select , #updateprice textarea').eq(1).focus();
              }
              else
              {
