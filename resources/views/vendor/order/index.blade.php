...
@endphp --}}
                                          ...
                                          <td>{{$qty}}</td>
                                      <td>{{$order->order->currency_sign}}{{round($order->order->pay_amount * $order->order->currency_value, 2)}}</td>
                                      ...
                                      <td class="">
                                        ...
                                        <a href="{{route('vendor-order-show',$order->order->order_number)}}" class="btn btn-primary product-btn"><i class="fa fa-eye"></i> {{ $langg->lang539 }}</a>
                                         ...
                                         @if(!empty(Auth::user()->localserve_email) && !empty(Auth::user()->localserve_password) && Auth::user()->localserve_email!=null && Auth::user()->localserve_password!=null)

                                             @if(is_null($order->order->api_combine_key)) 
                                              <a data-href="{{route('vendor-order-courier',$order->order->id)}}"  class="btn btn-primary sendToCourierbtn "> Request Courier</a>
                                              @else
                                              <button style="border-radius: 25px;"  class="btn btn-primary sendToCourierbtn " disabled=""> Courier Requested</button> 
                                              @endif

                                         @else                                                                               
                                         <a href="{{ route('vendor-profile') }}"  class="btn btn-primary"> Set Api's</a>
                                         @endif



                                            <select class="vendor-btn {{ $order->status }}">
                                            <option value="{{ route('vendor-order-status',['slug' => $order->order->id, 'status' => 'pending']) }}" {{  $order->order->status == "pending" ? 'selected' : ''  }}>{{ $langg->lang540 }}</option>
                                            <option value="{{ route('vendor-order-status',['slug' => $order->order->id, 'status' => 'processing']) }}" {{  $order->order->status == "processing" ? 'selected' : ''  }}>{{ $langg->lang541 }}</option>
                                            <option value="{{ route('vendor-order-status',['slug' => $order->order->id, 'status' => 'completed']) }}" {{  $order->order->status == "completed" ? 'selected' : ''  }}>{{ $langg->lang542 }}</option>
                                            <option value="{{ route('vendor-order-status',['slug' => $order->order->id, 'status' => 'declined']) }}" {{  $order->order->status == "declined" ? 'selected' : ''  }}>{{ $langg->lang543 }}</option>
                                            </select>
