            ...
            <div class="add-product-content">
              ...
              <div class="ap-product-categories">
                <div class="row sortable-posts">
                  @foreach($data->childs()->orderBy('position', 'ASC')->get() as $data)
                  <div id="{{ $data->id }}" class="col-lg-4 inid" style="margin-bottom: 20px;">
                    <a href="{{ route('vendor-prod-index1',[Request::route('category'),Request::route('subcategory'),$data->slug]) }}">
      ...
      $(".sortable-posts").sortable({
          stop: function() {
              $.map($(this).find('.inid'), function(el) {
                  ...
                  $.ajax({
                      url:"{{route('vendor-childcat-sort')}}",
                      type: 'GET',
                      data: {
                          id: id,
                          sorting: sorting
