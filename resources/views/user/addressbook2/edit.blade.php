...
<form id="userdashform" action="{{route('user.addressbook.edit',$addressbookdata->id)}}" method="post">
  ...
  <div class="mb-20">
    <div class="form-check  form-check-inline">
      <label class="container-radio">Shipping Address
        <input type="radio" name="address_type" {{$addressbookdata->address_type==0?'checked':''}} value="0">
        <span class="checkmark"></span>
      </label>
    </div>
    <div class="form-check ml-30 form-check-inline">
      <label class="container-radio">Billing Address
        <input type="radio"   name="address_type" {{$addressbookdata->address_type==1?'checked':''}} value="1">
        <span class="checkmark"></span>
      </label>
    </div>
  </div>
  ...
  <div class="row">
    <div class="col-md-6">
      ...
      <input type="text"  required="" class="form-control cust-adb-modal" name="fname" id="recipient-name" value="{{$addressbookdata->fname}}">
    </div>
    <div class="col-md-6">
     <label  class="col-form-label">Last Name:</label>
     <input type="text" required="" value="{{$addressbookdata->lname}}" class="form-control cust-adb-modal" name="lname">
    </div>

    <div class="col-md-6">
     <label  class="col-form-label">Phone</label>
     <input type="text" name="phone" value="{{$addressbookdata->phone}}" class="form-control cust-adb-modal" required="" >
    </div>


    <div class="col-md-6">
     <label class="col-form-label">Email</label>
     <input type="email" class="form-control cust-adb-modal" value="{{$addressbookdata->email}}" name="email" required="" >
    </div>
     ...
     <label class="col-form-label">Country/Region *</label>
      <select class="form-control cust-adb-modal" id="country" name="country" required="">
         <option selected="" disabled="">Country</option>
          ...
          <option value="{{$country->name}}" {{ $country->name == $addressbookdata->country ? "selected":"" }}>{{$country->name}}</option>
         @endforeach 
      </select>
    </div>
    ...
    <div class="col-md-6">
     <label  class="col-form-label">State/Province *</label>
      <select class="form-control cust-adb-modal" id="state" name="state" type= "text"  value="" required="">
         ...
         @foreach($addressbookdata->stat() as $state)
          <option value="{{$state->name}}" {{ $state->name == $addressbookdata->state ? "selected":"" }}>{{$state->name}}</option>
         @endforeach
      </select> 
    </div>
    ...
    <div class="col-md-6">
     ...
     <input type="text" class="form-control cust-adb-modal" name="city" value="{{$addressbookdata->city}}" required="">
    </div>


    <div class="col-md-6">
     <label class="col-form-label">Zip/Postal Code</label>
     <input type="text" class="form-control cust-adb-modal" value="{{$addressbookdata->zip}}" name="zip" required="" >
    </div>


    <div class="col-md-6">
      <label  class="col-form-label">Address line 1 <span style="font-size: 11px;font-weight: 600;">(Google Suggestions)</span> *</label>
      <input type="text" class="form-control cust-adb-modal" id="google_address" onfocus="initGoogleAddress()" value="{{$addressbookdata->address1}}" placeholder="Select Address from suggestions" name="address1" required="">
    </div>
    <div class="col-md-6">
     <label  class="col-form-label">Address line 2 (optional)</label>
     <input type="text" class="form-control cust-adb-modal" value="{{$addressbookdata->address2}}"  name="address2">
    </div>  
